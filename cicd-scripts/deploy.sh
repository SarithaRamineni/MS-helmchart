...
OUTPUT=$(helm history --max 1 $CI_PROJECT_NAME-$1 --namespace $1 2>/dev/null | grep failed | cut -f1 | grep 1)
EXPECTED_OUTPUT=1

if [ X"$OUTPUT" = X"$EXPECTED_OUTPUT" ]
then
  helm delete $CI_PROJECT_NAME-$1 --namespace $1
fi 

helm upgrade --install --set image.apiImage=$DOCKER_USERNAME/$CI_PROJECT_NAME:$CI_PIPELINE_ID-$1 --set deployment.environment=$1 $CI_PROJECT_NAME-$1 ./helm-chart --namespace $1